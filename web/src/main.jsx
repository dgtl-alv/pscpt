...
function App(){
  ...
  return <>
    <header className="topbar"><div className="brand"><span>PSCPT</span><small>PS Correlation Platform</small></div><nav>{user?<><button onClick={()=>setView('dashboard')}>Dashboard</button><button onClick={()=>setView('change')}>Change Password</button><button onClick={async()=>{await api('/api/auth/logout',{});setUser(null);setView('landing')}}>Logout</button></>:<><button onClick={()=>setView('landing')}>Home</button><button onClick={()=>setView('login')}>Login</button><button className="solid" onClick={()=>setView('register')}>Register</button></>}</nav></header>
    {msg && <div className="toast">{msg}</div>}
    {view==='landing' && <Landing go={setView}/>} {view==='login' && <Auth mode="login" done={authed}/>} {view==='register' && <Auth mode="register" done={authed}/>} {view==='forgot' && <Forgot setMsg={setMsg}/>} {view==='change' && <Change setMsg={setMsg}/>} {view==='dashboard' && <Dashboard user={user}/>} 
